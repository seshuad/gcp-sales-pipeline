...
spark.sql("CREATE DATABASE IF NOT EXISTS sales_processed")

# External tables over raw CSV data
spark.sql(f"""
...
spark.sql(f"""
    ...
    USING iceberg
    LOCATION '{PROCESSED}/revenue_by_region'
    TBLPROPERTIES (
...
spark.sql(f"""
    ...
    USING iceberg
    LOCATION '{PROCESSED}/revenue_by_product'
    TBLPROPERTIES (
        'write.format.default'            = 'parquet',
        'write.parquet.compression-codec' = 'snappy'
    )
...
spark.sql(f"""
    ...
    USING iceberg
    LOCATION '{PROCESSED}/monthly_trend'
    TBLPROPERTIES (
        'write.format.default'            = 'parquet',
        'write.parquet.compression-codec' = 'snappy'
    )
...
print("Done. Iceberg tables written to:", PROCESSED)
spark.stop()
